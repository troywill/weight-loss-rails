...
* [55%] Draw Google Graph
  ...
  6. [X] Use three day data array  file:../app/views/google_graph/three_day.html.erb
     #+BEGIN_SRC html
       - TDW var data = google.visualization.arrayToDataTable(<%= raw Reading.three_day_data_array(current_user).to_json %> );
     #+END_SRC
  8. [ ] Place a 28 day data array method in Readings controller
     - Commentary: :: We will pass data into Google javascript in the view
     - file:../app/models/reading.rb
       #+BEGIN_SRC ruby
         def self.month_array(user)
           weight = 0
           time_at_point_in_past = 0
           time_first_reading = Reading.time_initial(user)
           weight_first_reading = Reading.weight_initial(user).to_f
