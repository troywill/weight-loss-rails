* [5/5] Create a new rails application
  1. [8/8] Install Ruby on Rails 4
     1. [X] Install ruby 2.0.0p0 (2013-02-24 revision 39474)
     2. [X] sudo gem update (1m30s)
     3. [X] sprockets
	#+BEGIN_SRC sh
	  sudo gem install sprockets --prerelease --no-ri --no-rdoc
	#+END_SRC
     4. [X] sprockets-rails (17 gems installed)
	#+BEGIN_SRC sh
	  sudo gem install sprockets-rails --prerelease --no-ri --no-rdoc
	#+END_SRC
     5. [X] rails
	#+BEGIN_SRC sh
	...
	6. [ ] sudo gem install --local pkg/therubyracer-0.12.0beta1.gem
     8. [X] sudo gem install devise --version 3.0.0.rc
     9. [X] Run the 'rails' command as root
	#+BEGIN_SRC sh
          sudo rails new throwaway
        #+END_SRC
     10. Stuff that didn't work
	 ...
	 - sudo gem install therubyracer => no matching function for call to ‘v8::Context::SetData(rr::String)
  2. [X] Create an new rails application structure
  3. [X] Enable a JavaScript runtime in file:../Gemfile
     #+BEGIN_SRC ruby
  ...
  5. [X] Set the default route to home/index in file:../config/routes.rb
     #+BEGIN_SRC ruby
       root 'home#index'
     #+END_SRC
...
* [9/9] Incorporate Twitter Bootstrap see  [[file:/scpc:troy@usahealthscience.com:/home/troy/srv/bootstrap/128/kehoe/emacs/][~/srv/bootstrap/128/kehoe/emacs/]]
...
* [100%] Display current goal
  ...
  2. [X] View file:../app/views/home/index.html.erb
     #+BEGIN_SRC html
       ...
       <h1><%= number_with_precision(Goal.goal_now(current_user), :precision => 3)%></h1>
     #+END_SRC
* [100%] Weight as a function of time
  ...
  2. [X] In Reading model, self.get_readings_after, self.get_next_reading_after( user_id, time ) file:../app/models/reading.rb
     ...
     #+END_SRC
  3. [X] In Reading model, apply_filter file:../app/models/reading.rb
     #+BEGIN_SRC ruby
  ...
  6. [X] Display weight now in file:../app/views/home/index.html.erb
     ...
     #+END_SRC
* [100%] Draw Google Graph
  1. (Optional) See http://zargony.com/2012/02/29/google-charts-on-your-site-the-unobtrusive-way
  2. [X] Create a goal as a function of time method, place in Goal model file:../app/models/goal.rb
     #+BEGIN_SRC ruby
  ...
  3. [X] Generate the controller for generating Graphs
     ...
     #+END_SRC
  4. [X] Path for Google Graph three day in navigation file:../app/views/layouts/_navigation.html.erb layout
     #+BEGIN_SRC html
     ...
     #+END_SRC
  5. [X] Write a class `chart_array` method in Readings file:../app/models/reading.rb
     #+BEGIN_SRC ruby
     ...
     - Commentary: :: We will pass data into Google javascript in the view
  6. [X] VIEW Put Google Graph javascript into view a view partial three day file:../app/views/layouts/_google_graph.html.erb
     #+BEGIN_SRC html
  ...
  7. [X] Put google chart data array into controller file:../app/controllers/google_graph_controller.rb
     #+BEGIN_SRC ruby
     ...
     #+END_SRC
  8. [X] VIEW Call partial from 3 day, month, and 1 year views file:../app/views/google_graph/year.html.erb
     #+BEGIN_SRC html
       DEBUG chart data: <%= @google_chart_data_array %><br />
       DEBUG chart options: <%= @chart_options %>
       <%= render 'layouts/google_graph' %>
     #+END_SRC
* [100%] Display readings latest first
  1. [X] file:../app/controllers/readings_controller.rb ( welcome controller, index method )
     #+BEGIN_SRC ruby
  ...
  2. [X] VIEW file:../app/views/readings/index.html.erb
* [100%] SimpleForm
  1. [X] Install
     #+BEGIN_SRC sh
...
* [0%] Display readings table on Welcome Page
  ...
  - [ ] View
    + file:../../app/views/welcome/index.html.erb
      ...
      #+END_SRC
* [0%] Build a mailer to send messages to users 
  ...
  - [ ] Write stand alone mailer application
    ...
    #+END_SRC
* [0%] Weight loss/gain over the last 28 days
