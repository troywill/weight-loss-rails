...
* [8/8] Layout to work with Bootstrap
  ...
  8. [X] Deploy to Heroku
     #+BEGIN_SRC sh
       ...
       git push heroku master
     #+END_SRC
* [10/10] Create "Readings" model, controller, and views
  1. [X] Generate reading scaffold
     ...
     #+END_SRC
  2. [X] Limit readings to authenticated users [[file:../app/controllers/readings_controller.rb][app/controllers/readings_controller.rb]]
     #+BEGIN_SRC ruby
       class ReadingsController < ApplicationController
         before_filter :authenticate_user!
     #+END_SRC
  3. [X] Relationship to User, validations [[file:../app/models/reading.rb][app/models/reading.rb]]
     #+BEGIN_SRC ruby
       belongs_to :user
       validates :user_id, :numericality => true
       validates :weight, :numericality => true
     #+END_SRC
  4. [X] Default clothing in values in Model [[file:../db/migrate/][db/migrate/2012..._created_readings.rb]]
       #+BEGIN_SRC ruby
       ...
       #+END_SRC
  5. [X] Migrate the database, i.e. rake db:migrate 
  6. [X] [[file:../app/views/layouts/_navbar.html.erb][app/views/layouts/_navbar.html.erb]]
     #+BEGIN_SRC html
       <ul class="dropdown-menu">
     ...
     #+END_SRC
  7. [X] [100%] User ID on new Reading
  ...
  8. Reading Layout file:../app/views/layouts/readings.html.erb
     ...
     #+END_SRC
  9. [X] [[file:../app/controllers/readings_controller.rb][app/controllers/readings_controller.rb]] ( welcome controller, index method )
     #+BEGIN_SRC ruby
	@readings = Reading.where(:user_id => current_user.id).order('reading_time DESC')
     #+END_SRC
  10. [X] Bootstrap table [[file:../app/views/readings/index.html.erb][app/views/readings/index.html.erb]]
      #+BEGIN_SRC html
	<table class="table">
      #+END_SRC
  11. [X] Deploy to Heroku
      #+BEGIN_SRC sh
	...
	run rake db:migrate
      #+END_SRC
* [11/12] Create "Settings" model, controller, views, and default values
  1. [X] Generate setting scaffold
     #+BEGIN_SRC sh :tangle bin/generate-scaffold-setting.sh :shebang #!/bin/sh
       rails generate scaffold setting user_id:integer \
           ...
           locale:string
     #+END_SRC
  2. [X] Use readings layout file:../app/controllers/settings_controller.rb
     #+BEGIN_SRC ruby
       class SettingsController < ApplicationController
         layout "readings"
         before_action :set_setting, only: [:show, :edit, :update, :destroy]
     #+END_SRC
  3. [X] Validations [[file:../app/models/setting.rb][app/models/setting.rb]]
	#+BEGIN_SRC ruby
	 ...
	 #+END_SRC
  4. [X] Set default values in [[file:../db/migrate/][db/migrate 2013...._create_settings.rb]]
     #+BEGIN_SRC ruby
       t.integer :user_id
     ...
     #+END_SRC
  5. [X] Migrate database
  6. [X] Add current_user.id to create method [[file:../app/controllers/settings_controller.rb][app/controllers/settings_controller.rb]]
     #+BEGIN_SRC ruby
       def create
     ...
     #+END_SRC
  7. [X] Remove user_id from form [[file:../app/views/settings/_form.html.erb][app/views/settings/_form.html.erb]]
     #+BEGIN_SRC ruby
     ...
     #+END_SRC
  8. [X] Relationship between Setting and User [[file:../app/models/setting.rb][app/models/setting.rb]]
     #+BEGIN_SRC ruby
       class Setting < ActiveRecord::Base
         belongs_to :user
         ...
     #+END_SRC
  9. [X] Relationship between User and Setting [[file:../app/models/user.rb][models/user.rb]]
     #+BEGIN_SRC ruby
       class User < ActiveRecord::Base
         has_one :setting
     #+END_SRC
  10. [X] Add New Settings to be created when a new user is created [[file:../app/models/user.rb][app/models/user.rb]]
  ...
  12. [X] Deploy to Heroku
      #+BEGIN_SRC sh
        ...
        run rake db:migrate
      #+END_SRC
* [13/13] Create Goal model, controller, and views
  1. [X] Generate scaffold
     ...
     #+END_SRC
  2. [X] Layout [[file:../app/controllers/goals_controller.rb][app/controllers/goals_controller.rb]]
     #+BEGIN_SRC ruby
       class GoalsController < ApplicationController
         layout "readings"
     #+END_SRC
  3. [X] Table class [[file:../app/views/goals/index.html.erb][app/views/goals/index.html.erb]]
     #+BEGIN_SRC html
       <table class="table">
     #+END_SRC
  4. [X] Relationship between Goal and User [[file:../app/models/goal.rb][app/models/goal.rb]]
     #+BEGIN_SRC ruby
       class Setting < ActiveRecord::Base
         belongs_to :user
       ...
     #+END_SRC
  5. [X] Relationship between User and Goal [[file:../app/models/user.rb][app/models/user.rb]]
     #+BEGIN_SRC ruby
       class User < ActiveRecord::Base
         has_one :setting
         has_many :goals
     #+END_SRC
  6. [X] Default values [[file:../db/migrate/][db/migrate/ 2013xxx_create_goals.rb]]
     #+BEGIN_SRC ruby
       t.integer :goal_loss_rate, :default => 0
     #+END_SRC
  7. [X] Validations [[file:../app/models/goal.rb][app/models/goal.rb]]
     #+BEGIN_SRC ruby
     ...
     #+END_SRC
  8. [X] Migrate database
  9. [X] user_id [[file:../app/controllers/goals_controller.rb][app/controllers/goals_controller.rb]]
     #+BEGIN_SRC ruby
       # POST /goals
       # POST /goals.json
     ...
     #+END_SRC
  10. [X] update form [[file:../app/views/goals/_form.html.erb][app/views/goals/_form.html.erb]]
      remove user_id
  11. [X] navigation [[file:../app/views/layouts/_navbar.html.erb][app/views/layouts/_navbar.html.erb]]
      #+BEGIN_SRC html
        <!-- GOALS -->
        <li class="dropdown">
      ...
      #+END_SRC
  12. [X] Limit goals to authenticated users [[file:../app/controllers/goals_controller.rb][app/controllers/goals_controller.rb]]
      #+BEGIN_SRC ruby
        class GoalsController < ApplicationController
          before_filter :authenticate_user!
      #+END_SRC
  13. [X] Deploy to Heroku
      #+BEGIN_SRC sh
        git add app db
        git commit -a
        git push heroku master
...
* [3/3] Display current goal
  1. [X] goal_now in Goal model [[file:../app/models/goal.rb][app/models/goal.rb]]
     #+BEGIN_SRC ruby
     ...
     #+END_SRC
  2. [X] TODO goal_now helper file:../app/helpers/application_helper.rb
     #+BEGIN_SRC ruby
     ...
     #+END_SRC
  3. [X] View [[file:../app/views/home/index.html.erb][app/views/home/index.html.erb]]
     #+BEGIN_SRC html
       <% if current_user %>
       <h1>Your goal now :<%= goal_now %></h1>
       <% end %>
     #+END_SRC
* [8/8] Weight as a function of time
  1. [X] In Reading model, initial_reading function [[file:../app/models/reading.rb][app/models/reading.rb]] 
     #+BEGIN_SRC ruby
       def self.initial_reading( user )
         return Reading.order('reading_time ASC').where(:user_id => user.id).first
       end
     #+END_SRC
  2. [X] In Reading model, self.get_readings_after, self.get_next_reading_after( user_id, time ) [[file:../app/models/reading.rb][app/models/reading.rb]]
     #+BEGIN_SRC ruby
     ...
     #+END_SRC
  3. [X] In Reading model, apply_filter [[file:../app/models/reading.rb][app/models/reading.rb]]
     #+BEGIN_SRC ruby
     ...
     #+END_SRC
  4. [X] In Reading model, interpolate [[file:../app/models/reading.rb][app/models/reading.rb]]
     #+BEGIN_SRC ruby
	def self.interpolate( max_gain_rate, max_loss_rate, last_time, last_weight, next_time, next_weight, time )
      ...
      #+END_SRC
  5. [X] In Reading model, weight_at_time function [[file:../app/models/reading.rb][app/models/reading.rb]]
      #+BEGIN_SRC ruby
      ...
      #+END_SRC
  6. [X] Weight now helper file:../app/helpers/application_helper.rb
     #+BEGIN_SRC ruby
       def weight_now
         if current_user
     ...
     #+END_SRC
  7. [X] Display weight now in [[file:../app/views/home/index.html.erb][app/views/home/index.html.erb]]
     #+BEGIN_SRC ruby
...
* [10/10] Draw Google Graph
  1. (Optional) See http://zargony.com/2012/02/29/google-charts-on-your-site-the-unobtrusive-way
  2. [X] Create a goal as a function of time method, place in Goal model [[file:../app/models/goal.rb][app/models/goal.rb]]
     #+BEGIN_SRC ruby
     ...
     - Test with rails console
       ...
       #+END_SRC
  3. [X] Generate the controller for generating Graphs
     ...
     #+END_SRC
  4. [X] No turbolinks Google Graphs navigation [[file:../app/views/layouts/_navbar.html.erb][app/views/layouts/_navbar.html.erb]] layout
     #+BEGIN_SRC html
       <!-- Graphs -->
       <li class="dropdown">
         <a href="#" class="dropdown-toggle" data-toggle="dropdown">Graphs <b class="caret"></b></a>
     ...
     #+END_SRC
  5. [X] Write a class `chart_array` method in Readings [[file:../app/models/reading.rb][app/models/reading.rb]]
     #+BEGIN_SRC ruby
     ...
     - Test in rails console
       ...
       #+END_SRC
  6. [X] VIEW Put Google Graph javascript into view a view partial [[file:../app/views/layouts/_google_graph.html.erb][app/views/layouts/_google_graph.html.erb]]
     #+BEGIN_SRC html
  ...
  7. [X] Put google chart data array into controller [[file:../app/controllers/google_graph_controller.rb][app/controllers/google_graph_controller.rb]]
     #+BEGIN_SRC ruby
     ...
     #+END_SRC
  8. [X] VIEW Call partial from 3 day, month, and 1 year views [[file:../app/views/google_graph/][app/views/google_graph/]]
     #+BEGIN_SRC html
       DEBUG chart data: <%= @google_chart_data_array %><br />
       DEBUG chart options: <%= @chart_options %>
       <%= render 'layouts/google_graph' %>
     #+END_SRC
  9. [X] Simplify layout [[file:../app/controllers/google_graph_controller.rb][app/controllers/google_graph_controller.rb]]
     #+BEGIN_SRC ruby
       class GoogleGraphController < ApplicationController
         layout "readings"
     #+END_SRC
  10. [X] Limit graphs to authenticated users [[file:../app/controllers/google_graph_controller.rb][app/controllers/google_graph_controller.rb]]
      #+BEGIN_SRC ruby
        class GoalsController < ApplicationController
          before_filter :authenticate_user!
      #+END_SRC
