* [0%] Create a new rails application
  1. [ ] Run the 'rails' command
     #+BEGIN_SRC sh
  ...
  2. [ ] Enable a JavaScript runtime in file:../Gemfile
     ...
     #+END_SRC
  3. [ ] Create a “home” controller and a “home/index” page
     #+BEGIN_SRC sh
  ...
  4. [ ] Set the default route to home/index in file:../config/routes.rb
     #+BEGIN_SRC ruby
       root 'home#index'
     #+END_SRC
* [0%] Create a User authentication system with Devise < see [[file:/troy@usahealthscience.com:/home/troy/srv/devise/128/emacs/emacs/step-by-step-devise.org][step-by-step-devise.org]] >
...
* [0%] Create "Readings" model, controller, and views
  ...
  5. [ ] file:../app/views/layouts/_navigation.html.erb
     #+BEGIN_SRC html
...
* [0%] Create "Settings" model, controller, and views, default values
  1. [ ] Generate setting scaffold
     #+BEGIN_SRC sh :tangle bin/generate-scaffold-setting.sh :shebang #!/bin/sh
       rails generate scaffold setting user_id:integer \
           ...
           --force
     #+END_SRC
  2. [ ] Validations file:../app/models/setting.rb
	#+BEGIN_SRC ruby
	 ...
	 #+END_SRC
  3. [ ] Set default values in file:../db/migrate 2013...._create_settings.rb
     #+BEGIN_SRC ruby
       t.integer :user_id
  ...
  6. [ ] Remove user_id from form file:../app/views/settings/_form.html.erb
     ...
     #+END_SRC
  7. [ ] settings_path in application layout file:../app/views/layouts/_navigation.html.erb
...
* [0%] Create Goal model, controller, and views
  1. [ ] Generate scaffold
     ...
     #+END_SRC
  2. [ ] Relationship between Goal and User file:../app/models/goal.rb
     #+BEGIN_SRC ruby
       class Setting < ActiveRecord::Base
         belongs_to :user
       ...
...
* [0%] Display current goal
  1. [ ] goal_now in Goal model file:../app/models/goal.rb
     ...
     #+END_SRC
  2. [ ] View file:../app/views/home/index.html.erb
     #+BEGIN_SRC html
...
* [0%] Weight as a function of time
  ...
  2. [ ] In Reading model, self.get_readings_after, self.get_next_reading_after( user_id, time ) file:../app/models/reading.rb
     ...
     #+END_SRC
  3. [ ] In Reading model, apply_filter( max_gain_rate, max_loss_rate, initial_time, initial_weight, time, weight ) file:../app/models/reading.rb
     #+BEGIN_SRC ruby
...
* [0%] Draw Google Graph
  ...
  3. [ ] Path for Google Graph three day in navigation file:../app/views/layouts/_navigation.html.erb layout
     #+BEGIN_SRC html
        <li><%= link_to "3 day graph", google_graph_three_day_path %></li>
     #+END_SRC
  4. [ ] Place a 3 day data array method in Readings controller file:../app/models/reading.rb
     #+BEGIN_SRC ruby
  ...
  5. [ ] Put Google Graph javascript into view file:../app/views/google_graph/three_day.html.erb
     #+BEGIN_SRC html
  ...
  6. [ ] Use three day data array  file:../app/views/google_graph/three_day.html.erb
     #+BEGIN_SRC html
       - TDW var data = google.visualization.arrayToDataTable(<%= raw Reading.three_day_data_array(current_user).to_json %> );
     #+END_SRC
  6. [ ] Layout
     
  7. [ ] Place a 28 day data array method in Readings controller
     - Commentary: :: We will pass data into Google javascript in the view
     - file:../app/models/reading.rb
       #+BEGIN_SRC ruby
       ...
       #+END_SRC
  8. [ ] Create a goal as a function of time method, place in User model
     - file:../../app/models/user.rb
       ...
       #+END_SRC
  9. [ ] Add goal to month_array method in Reading model so it shows on the Google chart
