...
* [100%] Layout see  [[file:/scpc:troy@usahealthscience.com:/home/troy/srv/bootstrap/128/kehoe/emacs/][~/srv/bootstrap/128/kehoe/emacs/]]
  ...
  3. [X] New Application Layout with Twitter Bootstrap file:../app/views/layouts/application.html.erb
     ...
     #+END_SRC
* DONE [5/5] Create "Readings" model, controller, and views
  1. [X] Generate scaffold
     #+BEGIN_SRC sh :tangle bin/generate-scaffold-reading.sh :shebang #!/bin/sh
       rails generate scaffold reading user_id:integer weight:decimal reading_time:datetime clothing:boolean --force
     #+END_SRC
  ...
  5. [X] Add to file:../app/views/layouts/application.html.erb
     #+BEGIN_SRC html
       <li><%= link_to "All readings", readings_path %></li>
     #+END_SRC
* DONE [3/3] User ID on new Reading
  1. [X] Remove user id field from file:../app/views/readings/_form.html.erb
  2. [X] Remove breaks in form
  3. [X] Add user id to create method in readings controller
     file:../app/controllers/readings_controller.rb
     #+BEGIN_SRC ruby
       @reading.user_id = @user.id
     #+END_SRC
     - note that @user comes from application controller, identify_user method
* DONE [10/11] Create "Settings" model, controller, and views, default values
  1. [X] Generate scaffold
  ...
  5. [X] Add @user.id to create method
  ...
  6. [X] Remove user_id from form
     - file:../app/views/settings/_form.html.erb
       ...
       #+END_SRC
  7. [X] settings_path in application layout
  8. [ ] Fix numeric fields snafu
  9. [X] Remove link breaks from form
  10. [X] Relationship between Setting and User
      - file:../app/models/setting.rb
	#+BEGIN_SRC ruby
	  class Setting < ActiveRecord::Base
          ...
          ...
        #+END_SRC
  11. [X] Relationship between User and Setting
      - file:../app/models/user.rb
	#+BEGIN_SRC ruby
	  class User < ActiveRecord::Base
            has_one :setting
	#+END_SRC
* DONE Add New Settings to be created when a new user is created
  - file:../app/controllers/users_controller.rb
    ...
    #+END_SRC
* DONE [8/8] Create Goal model, controller, and views
  1. [X] Generate scaffold
     ...
     #+END_SRC
  2. [X] Relationship between Goal and User
	 ...
	 class Setting < ActiveRecord::Base
         ...
         ...
       #+END_SRC
  3. [X] Relationship between User and Goal
     - file:../app/models/user.rb
       #+BEGIN_SRC ruby
	 class User < ActiveRecord::Base
           has_one :setting
...
* DONE [2/2] Display current goal
** DONE goal_now in Goal model file:../app/models/goal.rb
   #+BEGIN_SRC ruby   
...
* DONE [5/5] Weight as a function of time
  1. [X] In Reading model, initial_reading function
     - file:../app/models/reading.rb
     #+BEGIN_SRC ruby
     ...
     #+END_SRC
  2. [X] In Reading model, self.get_readings_after, self.get_next_reading_after( user_id, time )
     - file:../app/models/reading.rb
     #+BEGIN_SRC ruby
     ...
     #+END_SRC
  3. [X] In Reading model, apply_filter( max_gain_rate, max_loss_rate, initial_time, initial_weight, time, weight )
     - file:../app/models/reading.rb
     #+BEGIN_SRC ruby
     ...
     #+END_SRC
  4. [X] In Reading model, interpolate
     - file:../app/models/reading.rb
     #+BEGIN_SRC ruby
...
* TODO [3/7] Draw Google Graph
  ...
  3. [X] Path for Google Graph three day in layout
     - file:../app/views/layouts/application.html.erb
     #+BEGIN_SRC html
        <li><%= link_to "3 day graph", google_graph_three_day_path %></li>
     #+END_SRC
  4. [X] Place a 3 day data array method in Readings controller
  ...
  8. [ ] Add goal to month_array method in Reading model so it shows on the Google chart
     - file:../../app/model/reading.rb
       ...
       #+END_SRC
* [2/2] Deploy to marv.usahealthscience.com
  1. [X] weight.usahealthscience.com
     1. [X] http://namecheap.com
     2. [X] All Host Records
        | SUB-DOMAIN | IP ADDRESS/URL  | RECORD TYPE |
        |------------+-----------------+-------------|
        | marv       | aaa.bbb.ccc.ddd | A(Address)  |
  2. [X] /etc/httpd/conf/httpd.conf (CentOS 6.4)
     1. [X] ServerName Directive
	#+BEGIN_SRC example
	  #ServerName www.example.com:80
	  ServerName marv.usahealthscience.com:80
	#+END_SRC
     2. [X] Restart Apache server
	#+BEGIN_SRC sh
	  httpd -k restart
	#+END_SRC
     3. [X] Stop Apache server
	#+BEGIN_SRC sh
	  httpd -k stop
	#+END_SRC
     4. [ ] Backup httpd.conf
...
** TODO [0/8] Draw a graph
 ...
 #+END_SRC
  11. [ ] In View:
	...
	#+END_SRC
** DONE [2/2] Create User model, controller, and view
   1. [X] Generate a `user` scaffold
      #+BEGIN_SRC sh
       	rails generate scaffold user name:string email:string
      #+END_SRC
   2. [X] Update the database
      #+BEGIN_SRC sh
       	rake db:migrate
      #+END_SRC
   #+END_SRC
** DONE [7/7] Identify the user
   1. [X] Add a before filter to the application controller
      ...
      - file:../app/controllers/application_controller.rb
          ...
          before_filter :identify_user, :except => :login
       	#+END_SRC
   2. [X] write a idenify_user method in application controller
      ...
      - file:../app/controllers/application_controller.rb
          ...
          def identify_user
       	...
       	#+END_SRC
   3. [X] Add a form to the login page
      - file:../app/views/welcome/login.html.erb
          ...
          <% end %>
       	#+END_SRC
   4. [X] Add a POST route for the login form
      - file:../config/routes.rb
          ...
          Weight::Application.routes.draw do
            ...
            ...
       	#+END_SRC
   5. [X] Add a login method to the welcome controller
      - file:../app/controllers/welcome_controller.rb
          ...
          def login
       	...
       	#+END_SRC
   6. [X] Add an authenticate method to the user model
      - file:../app/models/user.rb
       	#+BEGIN_SRC ruby
          ...
          def self.authenticate(email)
       	...
       	#+END_SRC
   7. [X] Write the logout method in the welcome controller
      - file:../app/controllers/welcome_controller.rb
       	#+BEGIN_SRC ruby
          def logout
            session[:user_id] = nil
